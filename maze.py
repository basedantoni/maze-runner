...
class Maze():
    ...
    def _solve_r(self, i, j):
        ...
        if i - 1 >= 0 and (not current_cell.has_left_wall and not self._cells[i - 1][j].has_right_wall) and not self._cells[i - 1][j].visited:
            current_cell.draw_move(self._cells[i - 1][j])
            if self._solve_r(i - 1, j):
                return True
            current_cell.draw_move(self._cells[i - 1][j], True)
        elif j - 1 >= 0 and (not current_cell.has_top_wall and not self._cells[i][j - 1].has_bottom_wall) and not self._cells[i][j - 1].visited:
            current_cell.draw_move(self._cells[i][j - 1])
            if self._solve_r(i, j - 1):
                return True
            current_cell.draw_move(self._cells[i][j - 1])
        elif i + 1 < self._num_cols and (not current_cell.has_right_wall and not self._cells[i + 1][j].has_left_wall) and not self._cells[i + 1][j].visited:
            current_cell.draw_move(self._cells[i + 1][j], True)
            if self._solve_r(i + 1, j):
                return True
            current_cell.draw_move(self._cells[i + 1][j], True)
        elif j + 1 < self._num_rows and (not current_cell.has_bottom_wall and not self._cells[i][j + 1].has_top_wall) and not self._cells[i][j + 1].visited:
            current_cell.draw_move(self._cells[i][j + 1])
            if self._solve_r(i, j + 1):
                return True
